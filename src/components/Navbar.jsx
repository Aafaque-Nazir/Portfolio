  ...
  return (
    <>
      <motion.nav
        ...
        className="navbar fixed top-0 left-0 w-full text-cyan-400 bg-transparent md:backdrop-blur-sm md:rounded-b-3xl z-50 flex justify-end md:justify-center mx-auto items-center p-6 md:p-4 md:shadow-xl"
      >
        {/* Hamburger Button - Clean & Transparent */}
        {!menuOpen && (
          <motion.button
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            whileTap={{ scale: 0.95 }}
            className="lg:hidden p-2.5 rounded-xl border border-white/10 bg-white/5 backdrop-blur-md text-cyan-400 shadow-[0_4px_20px_rgba(0,0,0,0.2)] hover:bg-white/10 hover:border-cyan-500/30 transition-all z-50"
            onClick={toggleMenu}
        ...
        <ul className="hidden lg:flex gap-7">
          {["Home", "About", "Projects", "Services", "Contact"].map((text) => (
            <li
              key={text}
              className="font-bold text-lg tracking-wider hover:scale-90 hover:animate-pulse transition-transform duration-400"
            >
              <a
