          ...
          {packages.map((pkg, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Card Content */}
                <div
                  className={`relative h-full flex flex-col p-8 rounded-3xl bg-[#0a0f1c]/90 backdrop-blur-xl border ${baseBorder} ${hoverBorder} shadow-2xl ${shadowColor} transition-all duration-300 overflow-hidden`}
                >
                  ...
                  {/* Title & Description */}
                  <h3 className="text-2xl font-bold text-white mb-2 tracking-wide group-hover:text-cyan-100 transition-colors">
                    {pkg.title}
                  </h3>
            ...
            {addons.map((addon) => (
              ...
              >
                <div className="p-8 flex items-start gap-6 relative z-10">
                  {/* Icon Box */}
                  <div
                    className={`p-4 rounded-2xl ${addon.bg} ${addon.color} text-2xl shadow-lg ring-1 ring-white/5`}
                  >
                    {addon.icon}
                  </div>
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-3">
                      ...
                      <span
                        className={`w-fit px-3 py-1 rounded-full text-xs font-bold border border-white/10 ${addon.color} ${addon.bg} uppercase tracking-wider`}
                      >
                        ₹{addon.price}
                      </span>
                    </div>
                ...
                {/* Hover Effects */}
                <div
                  className={`absolute inset-0 bg-gradient-to-r ${addon.bg.replace("/10", "/5")} opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none`}
                />
